...
class UserEncoder(nn.Module):
    ...
    def forward(self, news_histories: torch.Tensor, news_encoder: nn.Module) -> torch.Tensor:
        ...
        # Prepare attention mask if necessary (e.g., handle padding)
        attention_mask = torch.ones(batch_size, hist_size, device=news_histories_encoded.device)
        # Pass through FastformerEncoder
